...
from src.tp_utils.common import extract_product_ffs2

@pytest.fixture(scope="module")
def spark():
    return DatabricksSession.builder.getOrCreate()

def create_df_raw1(spark, values):
...
def test_valid_input(mock_logger, spark):
    ...
    value = "L12345678901234567890123456789012345678901234567890123456789012345678901234567890" * 2
    df_raw1 = create_df_raw1(spark, [value])
    df_max_lvl = create_df_max_lvl(spark, 1)

    result_df = extract_product_ffs2(df_raw1, 1, 2, 3, 4, df_max_lvl)
...
@patch("src.tp_utils.common.get_logger")
def test_no_L_rows(mock_logger, spark):
    mock_logger.return_value = MagicMock()
    df_raw1 = create_df_raw1(spark, ["X1234567890"])
    df_max_lvl = create_df_max_lvl(spark, 1)
...
def test_lvl_num_not_equals_max_lvl(mock_logger, spark):
    ...
    value = "L12345678901234567890123456789012345678901234567890123456789012345678901234567890" * 2
    df_raw1 = create_df_raw1(spark, [value])
    df_max_lvl = create_df_max_lvl(spark, 0)

    result_df = extract_product_ffs2(df_raw1, 1, 2, 3, 4, df_max_lvl)
    row = result_df.first()
    assert row is not None
    assert row["prod_match_attr_list"] != ""
