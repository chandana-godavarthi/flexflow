...
class TestSingleFileLoadMarketScript(unittest.TestCase):

    def setUp(self):
        self.script_path = "/home/runner/work/da-dp-pda-fftp01-databricks/da-dp-pda-fftp01-databricks/tp_dp_ff_pipelines/notebooks/src/t2_single_file_load_market.py"
    ...
    def get_mock_modules(self, run_id="R456", raise_exception=False):
        ...
        if raise_exception:
            mock_t2_load_file.side_effect = Exception("Market Load Failed")
        ...
        }, mock_common, mock_spark, mock_t2_load_file
    ...
    def test_successful_market_file_execution(self):
        mock_modules, mock_common, mock_spark, mock_t2_load_file = self.get_mock_modules()
        with patch.dict(sys.modules, mock_modules, clear=True):
            self.run_script(mock_spark)

        mock_t2_load_file.assert_called_once_with(
            "C123", "COMMON", "mkt", "R456", "load_market", "schema",
            mock_spark,
            "jdbc:postgresql://localhost:5432/refdb", "refdb", "user", "pwd"
        )

    def test_missing_run_id(self):
        mock_modules, mock_common, mock_spark, mock_t2_load_file = self.get_mock_modules(run_id=None)
        with patch.dict(sys.modules, mock_modules, clear=True):
            self.run_script(mock_spark)

        mock_t2_load_file.assert_called_once_with(
            "C123", "COMMON", "mkt", None, "load_market", "schema",
            mock_spark,
            "jdbc:postgresql://localhost:5432/refdb", "refdb", "user", "pwd"
        )

    def test_t2_load_file_exception(self):
        mock_modules, _, mock_spark, _ = self.get_mock_modules(raise_exception=True)
        with patch.dict(sys.modules, mock_modules, clear=True):
            with self.assertRaises(Exception) as context:
                self.run_script(mock_spark)

        self.assertIn("Market Load Failed", str(context.exception))