...
def test_load_measr_cntrt_factr_lkp_success():
    ...
    with patch("src.tp_utils.common.read_query_from_postgres", return_value=mock_df) as mock_read:
        result_df = common.load_measr_cntrt_factr_lkp(
        ...
        mock_read.assert_called_once_with(
            "SELECT * FROM public.MM_MEASR_CNTRT_FACTR_LKP",
            ANY,  # Spark
...
def test_load_measr_cntrt_factr_lkp_custom_schema():
    mock_df = MagicMock(name="DataFrame")
    with patch("src.tp_utils.common.read_query_from_postgres", return_value=mock_df) as mock_read:
        common.load_measr_cntrt_factr_lkp(
        ...
        assert "analytics.MM_MEASR_CNTRT_FACTR_LKP" in query

# Scenario 3: Exception during read
def test_load_measr_cntrt_factr_lkp_read_failure():
...
def test_load_measr_cntrt_factr_lkp_returns_dataframe():
    mock_df = MagicMock(name="DataFrame")
    with patch("src.tp_utils.common.read_query_from_postgres", return_value=mock_df):
        result = common.load_measr_cntrt_factr_lkp(
            postgres_schema="public",
            spark=MagicMock(name="SparkSession"),
            ref_db_jdbc_url="jdbc:postgresql://localhost:5432/refdb",
            ref_db_name="refdb",
