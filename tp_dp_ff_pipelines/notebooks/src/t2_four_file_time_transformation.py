...
from tp_utils.common import get_dbutils, read_run_params,load_cntrt_lkp,load_time_exprn_id,column_complementer, get_database_config,materialise_path,semaphore_generate_path,semaphore_acquisition,release_semaphore,safe_write_with_retry
...
if __name__ == "__main__":
    ...
    # Filter the DataFrame to get the 'Source System ID' for the specified contract ID
    ...
    logger.info(f"Source System ID: {srce_sys_id}, Time Expression ID: {time_exprn_id}")
    time_mat_path = materialise_path(spark)
   
    df_time_extrn=spark.read.parquet(f"{time_mat_path}/{run_id}/load_time_df_time_extrn")
    df_time_extrn.createOrReplaceTempView("df_time_extrn")
    logger.info("df_time_extrn")
    df_time_extrn.show()

    df_time_exprn = load_time_exprn_id(time_exprn_id,postgres_schema,spark, ref_db_jdbc_url, ref_db_name, ref_db_user, ref_db_pwd)
    logger.info("df_time_exprn")
    df_time_exprn.show()
    ...
    # Rename the 'perd_id' column to 'extrn_perd_id' and drop the 'perd_end_date' column 
    df_time_extrn=df_time_extrn.withColumnRenamed('perd_id','extrn_perd_id').drop('perd_end_date')
    ...
    #HS: add case when START DATE is set up, not END DATE, now END DATE join is hardcoded
    query = '''
    SELECT t.extrn_perd_id, fdim.time_perd_id, fdim.time_perd_end_date AS mm_time_perd_end_date, fdim.time_perd_type_code 
    FROM mm_time_extrn t 
    LEFT OUTER JOIN df_fdim fdim 
    ON (t.start_date_val = fdim.time_perd_start_date OR t.end_date_val = fdim.time_perd_end_date) AND t.time_perd_type_val = fdim.time_perd_type_code
    '''
    df_time_stgng_vw=spark.sql(query)
    logger.info("df_time_stgng_vw after join")
    df_time_stgng_vw.show()
    ...
    df_time_stgng_vw.write.mode("overwrite").format('parquet').save(f'''{time_mat_path}/{run_id}/time_transformation_df_time_stgng_vw''')
    logger.info(f"Time transformation Materialized to Path: {time_mat_path}/{run_id}/time_transformation_df_time_stgng_vw.parquet")
