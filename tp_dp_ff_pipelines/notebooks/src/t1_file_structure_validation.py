...
from tp_utils.common import read_run_params,get_dbutils,get_logger,read_from_postgres,tier1_file_structure_validation
...
if __name__ == '__main__':
    ...
    df_valdn=df_table.filter((df_table['run_id'] == run_id) & (df_table['valdn_grp_id'] == 5))

    if (df_valdn.count()==0 or df_valdn.collect()[0]['aprv_ind']!='Y'):
        print('Data Quality in progress')
        tier1_file_structure_validation(validation_name,file_name,cntrt_id,run_id,spark, ref_db_jdbc_url, ref_db_name, ref_db_user, ref_db_pwd,postgres_schema,catalog_name)
        
    df_valdn.show()

    # Filter the DataFrame for the specific run ID and validation name, and collect the data quality check result
    fail_ind = df_table.filter((df_table['run_id'] == run_id) & (df_table['valdn_grp_id'] == 5 )).collect()[0].fail_ind
    aprv_ind=df_table.filter((df_table['run_id'] == run_id) & (df_table['valdn_grp_id'] == 5 )).collect()[0].aprv_ind
