...
from tp_utils.common import get_dbutils, read_run_params,load_cntrt_lkp,read_from_postgres, get_database_config, tier1_business_validation, get_logger 

if __name__ == "__main__":
    spark = SparkSession.builder.appName("Tradepanel").getOrCreate()
    dbutils = get_dbutils(spark)
    logger = get_logger()
    validation_name = 'Business Validations'
    ...
    if any(check in lst_of_checks for check in ['CHK_DQ001','CHK_DQ002','CHK_DQ003','CHK_DQ004','CHK_DQ005','CHK_DQ006','CHK_DQ007']):
        bv_checks_enabled = 'Y'
    else:
        bv_checks_enabled = 'N'
    
    logger.info(f"bv_checks_enabled: {bv_checks_enabled}")
    #Execute the Business Validation only if the mastr_valdn_ind is active and atleast one of the Check is enabled
    if bv_checks_enabled=='Y':
        ...
        df_valdn=df_table.filter((df_table['run_id'] == run_id) & (df_table['valdn_grp_id'] == 8))
        ...
        if (df_valdn.count()==0 or df_valdn.collect()[0]['aprv_ind']!='Y'):
            ...
            tier1_business_validation(validation_name,file_name,cntrt_id,run_id,spark, ref_db_jdbc_url, ref_db_name, ref_db_user, ref_db_pwd,postgres_schema,catalog_name)
        else:
        ...
        fail_ind = df_table.filter((df_table['run_id'] == run_id) & (df_table['valdn_grp_id'] == 8 )).collect()[0].fail_ind
        aprv_ind=df_table.filter((df_table['run_id'] == run_id) & (df_table['valdn_grp_id'] == 8 )).collect()[0].aprv_ind
